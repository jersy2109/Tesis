...
def training(env_name, replay_memory_size=150_000, max_frames=10_000_000, gamma=0.99, batch_size=32,  \
    ...
    filename = env_name + "_DifOpt_" +  str(int(replay_memory_size/1_000)) + "k"
    path = "dicts/" + filename
    ...
    for frame in tqdm(range(1, max_frames+1), desc=filename):        
        ...
        if reward is not None:
            ...
            if best_mean_reward is None or best_mean_reward < mean_reward:
                torch.save(net.state_dict(), path + "/" + filename + "_DifBest.dat")
    ...
    pkl_file = "dicts/" + filename + "/" + filename + "_DifTotal.pkl"
    with open(pkl_file, 'wb+') as f:
        pickle.dump(total_rewards, f)
    pkl_file = "dicts/" + filename + "/" + filename + "_DifLoss.pkl"
    with open(pkl_file, 'wb+') as f:
    ...
    aux_file = "dicts/" + filename + "/" + filename + "_DifParameters.txt"
...
def sample_model(game, samples=30, directory=None):
    ...
    pkl_file = "samples/" + game + "_DifSample_rewards.pkl"
    with open(pkl_file, 'wb+') as f:
...
if __name__ == '__main__':
    ...
    path = "dicts/" + GAME + "_DifOpt_" +  str(int(SIZE/1_000)) + "k"
