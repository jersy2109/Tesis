...
def training(env_name, replay_memory_size=150_000, max_frames=10_000_000, gamma=0.99, batch_size=32,  \
    ...
    filename = env_name + "_OptT_" +  str(int(exp_frames/1_000)) + 'kFrames_' + str(int(replay_memory_size/1_000)) + "k_" + str(int(max_frames/1_000_000)) + 'M'
    path = "dicts/" + filename
...
def sample_model(game, samples=30, directory=None):
    ...
    if '500kFrames' in directory:
        pkl_file = "samples/" + game + '_OPT_sample_rewards_' + directory.split('_')[-1] + '_T.pkl' #game + "_sample_rewards_1M_T.pkl"
    else:
        pkl_file = "samples/" + game + '_OPT_sample_rewards_' + directory.split('_')[-1] + '.pkl'
    with open(pkl_file, 'wb+') as f:
