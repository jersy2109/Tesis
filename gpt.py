...
def make_atari(env_id, **params):
    env = gym.make(env_id, render_mode=None)
    assert 'NoFrameskip' in env.spec.id
    env = NoopResetEnv(env, params['noop_max'])
    env = MaxAndSkipEnv(env, params['skip'])
...
def training(env_name, **params):
    filename = ''.join(params['path'].split('/')[1:])
    Path(params['path']).mkdir(parents=True, exist_ok=True)

    env = make_atari(env_name + 'NoFrameskip-v4', **params)
    buffer = ExperienceReplay(params['replay_memory_size'])
    agent = Agent(env, buffer)
    set_seed(seed=params['seed'], env=env)

    net = DQN(env.observation_space.shape, env.action_space.n).to(device)
    target_net = DQN(env.observation_space.shape, env.action_space.n).to(device)
    
    epsilon = params['eps_start']
    eps_decay = (params['eps_start'] - params['eps_min']) / params['exp_frames']
    ...
    for frame in tqdm(range(1, params['max_frames'] + 1), desc=filename):
        ...
        if reward is not None:
            ...
            if best_mean_reward is None or best_mean_reward < mean_reward:
                torch.save(net.state_dict(), params['path'] + "/" + filename + "_best.dat")
        ...
        epsilon = max(epsilon - eps_decay, params['eps_min'])

        if frame % params['net_update'] == 0:
            sardn = buffer.sample(params['batch_size'])
            ...
            rewrads_v = torch.tensor(batch.reward).to(device)
            ...
            expected_state_action_values = next_state_values * params['gamma'] + rewrads_v
        ...
        if frame % (params['max_frames'] // params['numberOfDicts']) == 0:
            if params['verbose']:
            ...
            torch.save(net.state_dict(), params['path'] + "/" + filename + '_' + str(int(frame / (params['max_frames'] // params['numberOfDicts']))) + "k.dat")
    ...
    parameters = "Environment: {} \
                ...
                \nRandom Seed: {} \
                \nFinished Training: {} \
                \nTraining Time: {}".format(env_name, params['replay_memory_size'], params['max_frames'], params['exp_frames'],
                                            params['gamma'], params['batch_size'], params['learning_rate'],
                                            params['sync_target_frames'], params['net_update'], params['replay_start_size'],
                                            params['eps_start'], params['eps_min'], params['seed'], tr_finished, end_time)
...
def sample(game, modelDir, model_name, n_samples=30, verbose=True):
    ...
    env = make_atari(game, **sample_params)
    net = DQN(env.observation_space.shape, env.action_space.n)
    net.load_state_dict(torch.load(model, map_location=lambda storage, loc: storage))
    epsilon = sample_params['epsilon'] 
